...
def met_recurrent_val_threshold(p,name):
    vm=val_mask & rec; pv=p[vm]; yv=y[vm]; best=(.5,-1)
    for t in np.unique(np.quantile(pv,np.linspace(0,1,501))):
        _,_,f,_=precision_recall_fscore_support(yv,pv>=t,average='binary',zero_division=0)
        if f>best[1]: best=(float(t),float(f))
    tm=test_mask & rec; yt=y[tm]; pt=p[tm]; pred=pt>=best[0]
    pr,re,f,_=precision_recall_fscore_support(yt,pred,average='binary',zero_division=0)
    return {'name':name,'val_recurrent_n':int(vm.sum()),'val_recurrent_positives':int(yv.sum()),'threshold':best[0],'val_best_f1':best[1],'test_n':int(tm.sum()),'test_positives':int(yt.sum()),'pr_auc':float(average_precision_score(yt,pt)),'roc_auc':float(roc_auc_score(yt,pt)),'precision':float(pr),'recall':float(re),'f1':float(f)}
res=[met(pstatic,'static_robust_all'),met(pgru,'temf_gru_v0_all'),met(pstatic,'static_robust_recurrent_global_threshold',rec),met(pgru,'temf_gru_v0_recurrent_global_threshold',rec),met_recurrent_val_threshold(pstatic,'static_robust_recurrent_local_threshold'),met_recurrent_val_threshold(pgru,'temf_gru_v0_recurrent_local_threshold')]
np.savez(base+'/elliptic_temf_gru_v0_predictions.npz',pgru=pgru,pstatic=pstatic,times=times,y=y,prior_count=prior_count)
